...
function validateFrame(frame) {
    ...
    for (let index = 1; index < frame.rows.length; index += 1) {
        if (frame.rows[index - 1].absoluteRow >= frame.rows[index].absoluteRow)
            throw new Error('terminal-output-plugin: rows must be ordered');
    }
}
export class TuiTerminalOutputService extends Service {
    ...
    reset(sessionKey) {
        if (this.disposed)
            throw new Error('terminal-output-plugin: disposed');
    ...
    apply(frame) {
        ...
        for (const [absoluteRow, previous] of previousRows) {
            ...
            if (next === undefined || rowSignature(previous) !== rowSignature(next))
                dirty.add(absoluteRow);
        }
        for (const [absoluteRow, next] of nextRows) {
            const previous = previousRows.get(absoluteRow);
            if (previous === undefined || rowSignature(previous) !== rowSignature(next))
                dirty.add(absoluteRow);
        }
    ...
    read() { if (this.disposed)
        throw new Error('terminal-output-plugin: disposed'); return this.snapshot; }
